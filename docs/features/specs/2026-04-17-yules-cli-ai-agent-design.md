...
4. **Packaging:** `npm run build` then `npm i -g .` (or `npm link`); the binary name is **`yules-ai`**.
...
| `package.json`               | `dependencies` for `ai`, `@ai-sdk/openai`, `dotenv`; `"bin": { "yules-ai": "dist/cli.js" }` (exact path must match compiled output).                                       |
...
- **Install:** `npm run build`, then `npm i -g .` — `yules-ai` on `PATH`.

## Verification

1. `npm run build` succeeds.
2. From a directory containing a valid `.env` with `OPENAI_API_KEY`, running `yules-ai` starts the REPL, streams replies, and maintains context across turns within one process.
3. Running from a directory **without** a usable key fails before the REPL with a clear message.

## Notes

- **Global install + cwd `.env`:** Developers run `yules-ai` from a project folder that contains their `.env`; no requirement to put secrets beside the installed package.
