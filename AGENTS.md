...
**Current entry:** `src/cli.ts` loads env, validates `OPENAI_API_KEY`, and renders the Ink UI (`src/ui/App.tsx`). Streaming chat uses `src/agent/chat.ts` (`streamText`) and `src/ui/useAgentChat.ts` for state.
...
| `src/ui/useAgentChat.ts`     | Chat state, streaming consumption, errors                       |
