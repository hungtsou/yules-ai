...
**Current entry:** `src/cli.ts` loads env, validates `OPENAI_API_KEY`, and renders the Ink UI (`src/ui/App.tsx`). Streaming chat uses `src/agent/run.ts` (`streamText` via `runAgent`) and `src/ui/useAgentChat.ts` for state.
